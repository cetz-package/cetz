...
/// Find roots of a cubic bezier for the coefficients given
///
/// -> array Array of t values
#let _cubic-roots(a, b, c, d) = {
  ...
  if D >= 0 {
    ...
    let aa = -A / 3
    let roots = (aa + (S + T), aa - (S + T) / 2, aa - (S + T) / 2)
    if (S - T) != 0 {
...
#let cubic-line-intersect(s, e, c1, c2, la, lb, ray: false) = {
  // Based on:
