...
#test-case({
  ...
  polygon((0, 0), 3, radius: 2cm)
})

#test-case({
  import cetz.draw: *

  polygon((0, 0), 5, radius: 2, name: "poly")

  for-each-anchor("poly", name => {
    if name.starts-with(regex("(corner|edge)")) {
      point((), [#name], fill: blue)
    }
  })
})
...
#test-case({
  ...
  polygon((0, 0), 5, radius: 2, name: "poly", angle: 30deg)
  show-compass-anchors(element: "poly")
})
...
#test-case({
  ...
  polygon((0, 0), 5, radius: 2, name: "poly")
  show-border-anchors(element: "poly")
})
