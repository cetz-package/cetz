...
<Parameter name="angle" types="angle">
  The angle of the coordinate. An angle of `0deg` is to the right, a degree of
  `90deg` is upward.
</Parameter>

<Parameter name="radius" types="number,array">
  The distance from the origin. An array of <Type>number</Type> can be given, in
  the form `(x, y)`, to define the `x` and `y` radii of an ellipse instead of a
  circle.
</Parameter>
...
>
  ...
  anchor will be used, on most elements this is `center` but it can be different
  or not exist at all!
</Parameter>
